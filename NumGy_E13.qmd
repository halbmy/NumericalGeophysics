---
title: "Numerical Simulation Methods in Geophysics, Exercise 13:<br> 1D/2D EM"
...
$$ H = H_0 e^{-\imath k z} \qqtext{with} k^2=-\imath\omega\mu\sigma $$
...
T = 0.1  # 1, 10
w = 2 * np.pi / T
sigma0 = 1/100  # 100 Ohmm, 10, 1000
mu = np.pi * 4e-7
d = np.sqrt(2/(mu*w*sigma0))
print(d)
z = np.arange(0, 100001, 100)

def Hanalytical(z, sigma, w):
...
A, b = stiffnessMatrix1DFE(x=z, uL=1)
...
## Complex-to-real conversion (numpy)
...
## Complex-to-real conversion (pyGIMLi)
...
## Secondary field in matrix form
...
## Secondary field for H field

$$-\div \rho_0 \grad \vb H_0 +\imath\omega\mu \vb H_0=0$$

leads to

$$ -\div \rho \grad \vb H_a + \imath\omega\mu \vb H_a = \div \delta\rho_0 \grad \vb H_0 $$

$$ \vb A_\rho \vb u_a + \imath\omega\vb M \vb u_a = (\vb A_\rho + \imath\omega\vb M) \vb u_a =
-\vb A_{\delta\rho} \vb u_0 $$
